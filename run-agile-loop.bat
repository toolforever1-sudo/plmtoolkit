...
REM ---- Auto-promote a newer staging jar to live (2026-07-15) ----------------
REM Delegates to promote-agile.bat (same folder), which is the proven promoter:
REM if staging\plm-agile-service-1.0.0.jar is newer than the live jar it backs
REM up live and copies staging over. "nopause" runs it non-interactively.
REM Its console output appears in this watchdog window. The live jar must be
REM free (JVM stopped) for the copy to land - this runs before we launch below.
echo [%date% %time%] checking for newer staging jar (promote-agile.bat) >> "%WATCHDOG_LOG%"
if exist "%~dp0promote-agile.bat" (
    call "%~dp0promote-agile.bat" nopause
) else (
    echo [%date% %time%] promote-agile.bat not found - skipping auto-promote >> "%WATCHDOG_LOG%"
)
REM --------------------------------------------------------------------------
