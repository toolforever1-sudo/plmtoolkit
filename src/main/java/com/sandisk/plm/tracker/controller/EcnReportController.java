...
public class EcnReportController {
    ...
    public Map<String, Object> runReport(HttpSession session) {
        ...
        // server-wide. Prevents overlapping ecn_report_generator.py runs from
        // pegging the box CPU (prod incident). Reject fast if one is running.
        final String pySlotLabel = "ecn-report:" + user;
        if (!reportService.tryAcquirePythonSlot(pySlotLabel)) {
            String holder = reportService.getPythonSlotHolder();
            Map<String, Object> busy = new LinkedHashMap<>();
            busy.put("success", false);
            busy.put("message", "A report is already generating"
                    + (holder != null ? " (" + holder + ")" : "")
