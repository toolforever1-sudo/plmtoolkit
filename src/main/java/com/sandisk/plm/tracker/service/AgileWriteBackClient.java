...
public class AgileWriteBackClient {
    ...
    // helpers
    // ------------------------------------------------------------------
    /** GET with up-to-3 retries on transient failure. */
    private Result getWithRetry(String url, String cid, String action) {
        ...
        while (attempts < 3) {
            ...
            try {
                ...
                if (last.httpStatus == 404 || last.httpStatus == 409) return last; // not transient
            } catch (Exception e) {
                last = clientError(cid, action, e, System.currentTimeMillis() - t0);
            } finally {
                if (conn != null) conn.disconnect();
            }
            // backoff: 2s, 4s, 8s
