...
public class ImsReviewPdfService {
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            if (in.dcoForm != null && !in.dcoForm.isEmpty()) {
                ...
                try (PDPageContentStream cs2 = new PDPageContentStream(doc, page2)) {
                    ...
                    // Section 04 of the form — the five SDSM document attributes the
                    // responder can edit. These are written straight onto the Agile
                    // document, so the attestation PDF must record what was changed.
                    // Omitted entirely when nothing was edited (most documents).
                    if (hasSdsmEdits(ff)) {
                        y2 -= 8;
                        y2 = drawSectionLabel(cs2, sectionFont, marginX, y2, "SDSM document attributes written to Agile");
                        y2 = drawKV(cs2, bodyFont, bodyFont, marginX, y2, lineGap, "VAR Type",      formStr(ff, "varType"), false);
                        y2 = drawKV(cs2, bodyFont, bodyFont, marginX, y2, lineGap, "Product Group", formStr(ff, "productGroups"), false);
                        y2 = drawKV(cs2, bodyFont, bodyFont, marginX, y2, lineGap, "Spec",          formStr(ff, "specs"), false);
                        y2 = drawKV(cs2, bodyFont, bodyFont, marginX, y2, lineGap, "Product",       formStr(ff, "products"), false);
                        y2 = drawKV(cs2, bodyFont, bodyFont, marginX, y2, lineGap, "PM Checklist",  formStr(ff, "pmChecklist"), false);
                    }
    ...
    /** Placeholder for an empty field. Plain ASCII — see {@link #asciify}. */
    private static final String EMPTY = "-";

    /** Form keys for the editable SDSM document attributes (change-order form §04). */
    private static final String[] SDSM_EDIT_KEYS =
            { "varType", "productGroups", "specs", "products", "pmChecklist" };

    /** True when the responder set at least one SDSM attribute — i.e. the change
     *  order actually edits the document, so the PDF should record the new values. */
    private static boolean hasSdsmEdits(java.util.Map<String, Object> form) {
        for (String k : SDSM_EDIT_KEYS) if (isPresent(form, k)) return true;
        return false;
    }

    private static boolean isPresent(java.util.Map<String, Object> form, String key) {
        Object v = form == null ? null : form.get(key);
        if (v == null) return false;
        if (v instanceof java.util.List) return !((java.util.List<?>) v).isEmpty();
    ...
    private static String formStr(java.util.Map<String, Object> form, String key) {
        ...
        String s = personStr(v);
        return s.isEmpty() ? EMPTY : s;
    }

    /**
     * Render one list element. People (Document Owners / Approvers / Observers)
     * arrive as {@code {loginId, displayName, email}} maps, and the old code
     * called {@code toString()} on them — so the approval PDF literally printed
     * {@code {loginId=1000296585, displayName=Singh, Vikas, email=…}} in the
     * PEOPLE block (reported by Vikas Singh 2026-07-09). Render "Name (loginId)"
     * instead, falling back to whatever identifying field is present.
     */
    private static String personStr(Object o) {
        if (o == null) return "";
        if (o instanceof java.util.Map) {
            java.util.Map<?, ?> m = (java.util.Map<?, ?>) o;
            String name  = str(m.get("displayName"));
            String login = str(m.get("loginId"));
            String email = str(m.get("email"));
            if (!name.isEmpty())  return login.isEmpty() ? name : name + " (" + login + ")";
    ...
    /**
     * Make a string safe for PDFBox's standard-14 fonts.
     *
     * <p>Those fonts can't encode anything outside Latin-1, and the old code
     * mapped every such character to a literal {@code "?"}. Because our copy is
     * typographic — em-dashes in "Sandisk — IMS Document Review" and
     * "Approval — Needs Change — DCO Submitted via Toolkit", an ellipsis in the
     * fingerprint, and an em-dash as the empty-field placeholder — the generated
     * approval PDF was peppered with question marks (reported by Vikas Singh
     * 2026-07-09). Transliterate to the obvious ASCII equivalent instead, and
     * only fall back to '?' for something genuinely unrepresentable.
     */
    static String asciify(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            switch (c) {
                ...
                // NB: '·' (U+00B7) is Latin-1 and already renders — leave it alone.
                ...
                case '✓': case '✔': sb.append("OK");   break;                // ✓ ✔
                default:
                    // Latin-1 printable range is safe; C1 controls (0x80-0x9F) are not.
                    if (c <= 0x7E || (c >= 0x00A1 && c <= 0x00FF)) sb.append(c);
                    else if (c == '\n' || c == '\r' || c == '\t') sb.append(' ');
                    else sb.append('?');
            }
        }
    ...
    /** Same transliteration as {@link #drawText} so wrap/width maths matches what is drawn. */
