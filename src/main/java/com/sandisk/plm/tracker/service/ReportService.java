...
public class ReportService {
    ...
    // -----------------------------------------------------------------------
    // Global heavy-Python concurrency guard.
    // Prod incident: two ecn_report_generator.py running at once pegged the
    // box CPU and starved the JVM (23s IMS SQL pulls, 5min LDAP enrich, a
    // deploy that took 13 min instead of ~1). Each heavy Python report must
    // acquire this single slot before spawning python.exe, and release it in
    // a finally block. Overlapping triggers now queue (or are rejected) rather
    // than stacking up.
    // -----------------------------------------------------------------------
    private final Semaphore pythonSlot = new Semaphore(1, true);
    private final AtomicReference<String> pythonHolder = new AtomicReference<>(null);
    private volatile long pythonHeldSince = 0L;

    /** Try to grab the single heavy-Python slot, waiting up to waitSeconds.
     *  Returns true if acquired (caller MUST call releasePythonSlot in a
     *  finally block). Returns false if another report holds it past the wait. */
    public boolean tryAcquirePythonSlot(String label, long waitSeconds) {
        try {
            boolean got = pythonSlot.tryAcquire(waitSeconds, TimeUnit.SECONDS);
            if (got) {
                pythonHolder.set(label);
                pythonHeldSince = System.currentTimeMillis();
                logger.info("[PYSLOT] acquired by " + label);
            } else {
                logger.warning("[PYSLOT] " + label + " could not acquire \u2014 held by "
                        + pythonHolder.get() + " for "
                        + ((System.currentTimeMillis() - pythonHeldSince) / 1000) + "s");
            }
            return got;
    ...
    /** Convenience: no-wait acquire. */
    public boolean tryAcquirePythonSlot(String label) {
        return tryAcquirePythonSlot(label, 0);
    }

    public void releasePythonSlot(String label) {
        // Only release if this label still appears to hold it (avoid over-release).
        if (pythonHolder.compareAndSet(label, null)) {
            pythonHeldSince = 0L;
            pythonSlot.release();
            logger.info("[PYSLOT] released by " + label);
        }
    }

    /** Who currently holds the Python slot (or null if free) \u2014 for status/UI. */
    public String getPythonSlotHolder() {
        return pythonHolder.get();
    }
