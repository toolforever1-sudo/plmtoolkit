...
// PT-125 (Jimmy Sessumes, 2026-07-14): on the Cycle Time (Product Team) YTD
// panel specifically, any % on Target below 90% should read red. The shared
// ecnPctColor (80% threshold) is intentionally left alone so the General/PDR
// cycle-time panels, POM, headline strip, and the emailed report keep their
// existing 80% coloring. This helper is used ONLY by ecnRenderTeamPanel.
function ecnTeamPctColor(pct) {
    if (pct === null || pct === undefined) return '#6B7280';
    return pct >= 90 ? '#1F8A4C' : '#B8342B';
}
...
function ecnRenderTeamPanel(teams, trends) {
    ...
    for (var i = 0; i < limit; i++) {
        ...
        rows.push(ecnKpiRow(t.team, [
            ...
            { text: t.pctOnTarget !== null ? t.pctOnTarget + '%' : 'NA', color: ecnTeamPctColor(t.pctOnTarget) }
        ]));
        rows.push(ecnKpiRow('\u00a0\u00a0Standard', [
            { text: t.standard.count }, { text: t.standard.pct + '%' }, { text: t.standard.avgDays },
            { text: t.standard.pctOnTarget !== null ? t.standard.pctOnTarget + '%' : 'NA', color: ecnTeamPctColor(t.standard.pctOnTarget) }
        ]));
        rows.push(ecnKpiRow('\u00a0\u00a0Urgent', [
            { text: t.urgent.count }, { text: t.urgent.pct + '%' }, { text: t.urgent.avgDays },
            { text: t.urgent.pctOnTarget !== null ? t.urgent.pctOnTarget + '%' : 'NA', color: ecnTeamPctColor(t.urgent.pctOnTarget) }
        ]));
    }
