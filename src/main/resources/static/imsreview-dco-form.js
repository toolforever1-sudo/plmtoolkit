...
(function () {
  ...
  // Inline "SDSM only" hint, sitting to the right of the Business Unit buttons
  // (Vikas Singh 2026-07-09 — the scope of this field wasn't obvious).
  function sdsmOnlyNote() {
    return '<span style="margin-left:12px; font-size:11px; color:#6B7280; white-space:nowrap;">'
         + 'Only applies to SDSM Documents</span>';
  }
  // NB: the old prefillNoteText()/prefillNoteDiv() helpers are gone — the
  // "auto-filled from the document" note now lives in each field's ⓘ tooltip
  // (Product Line, Subcontractors) rather than under the control.
  ...
  function renderForm() {
    ...
    var sec02 = coSection('02', 'Scope',
        coRow('Product Line(s)', true,
            ...
            'Auto-filled from Document Title Block, may be added or removed as needed.')
      + coRow('Subcontractors', true,
            subcontractorPickerHtml(m.subcontractors || []),
            // Auto-fill note lives in the tooltip, not under the picker (Vikas Singh 2026-07-09).
            'Auto-filled from Document Title Block, may be added or removed as needed.')
      ...
      + coRow('Change Impact Disposition', true,
            ...
            'Derived from ISO 9001:2015 Clause 6.3, requiring planned changes with consideration of impact, QMS integrity, resources, and responsibilities.')
    ...
    var sec03 = coSection('03', 'People',
        coRow('Document Owner(s)', true,
            typeaheadHtml('documentOwners'),
            // Prefill note moved into the tooltip (Vikas Singh 2026-07-09).
            'Pre-filled from the document. Add or remove as needed.')
      + coRow('Approvers', true, typeaheadHtml('approvers'),
            'To ensure the document is accurate, compliant with IMS and business requirements, and fit for implementation.')
      + coRow('Observers', false, typeaheadHtml('observers'),
            'To ensure awareness of changes affecting their function or responsibilities and promote cross-functional alignment and understanding.'));
    ...
    var sec04 = coSection('04', 'SDSM IMS Document — Edit',
        ...
        'This section only applies to SDSM Documents');
    ...
    var stakeholderTip = 'To keep the right people informed and aligned when changes occur — fill **at least one** of Email Copy or Email Addresses below.';
  ...
  function fslotHtml(label, hint, key) {
    var f = DCO.files[key];
    var hasFile = !!f;
    var displayHint = hasFile ? esc(f.name) + ' · ' + (f.size / 1024 / 1024).toFixed(2) + ' MB' : esc(hint) + ' <span style="opacity:.65;">· or drop file here</span>';
  ...
  function mfslotHtml(label, hint, key) {
    ...
    return '<div class="dco-fslot' + (arr.length ? ' has-file' : '') + '" style="flex-wrap:wrap;" data-key="' + key + '">'
         ...
         +     (arr.length
                 ...
                 : '<div class="dco-fslot-hint">' + esc(hint) + ' <span style="opacity:.65;">· or drop files here</span></div>')
         +   '</div>'
  ...
  // File slots
  // -------------------------------------------------------------------------
  // Shared accept path for both the file picker and drag-and-drop. Multi-file
  // slots (arrays) append every file; single slots take the first.
  function acceptFilesForKey(key, fileList) {
    if (!fileList || !fileList.length) return;
    captureFormState();   // preserve in-progress text before the rebuild
    if (Array.isArray(DCO.files[key])) {
      for (var i = 0; i < fileList.length; i++) DCO.files[key].push(fileList[i]);
    } else {
      DCO.files[key] = fileList[0];
    }
    renderForm();
  }
  ...
  function bindFileSlot(key) {
    var inp = $('dco-file-' + key);
    if (!inp) return;
    inp.addEventListener('change', function () { acceptFilesForKey(key, inp.files); });

    // Drag-and-drop onto the slot box (works for single + multi slots).
    // renderForm() rebuilds the DOM, so each render binds a fresh element.
    var slot = document.querySelector('.dco-fslot[data-key="' + key + '"]');
    if (slot) {
      var quell = function (e) { e.preventDefault(); e.stopPropagation(); };
      slot.addEventListener('dragenter', function (e) { quell(e); slot.classList.add('dco-dragover'); });
      slot.addEventListener('dragover',  function (e) { quell(e); slot.classList.add('dco-dragover'); });
      slot.addEventListener('dragleave', function (e) { quell(e); slot.classList.remove('dco-dragover'); });
      slot.addEventListener('drop', function (e) {
        quell(e);
        slot.classList.remove('dco-dragover');
        var dt = e.dataTransfer;
        if (dt && dt.files && dt.files.length) acceptFilesForKey(key, dt.files);
      });
    }
  }
