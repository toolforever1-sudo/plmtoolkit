...
(function () {
    ...
    var _state = {
        ...
        globalLocator: '',       // PT-72: cross-tile search by Doc# / DRR# / DCO# — when set, tile filter is bypassed
        ...
        // Action History sub-tab (fetched lazily on first activation)
        actionLog: null,         // full /action-log payload, cached
        ahLoading: false,
        ahCat: 'all',            // 'all' | 'send' | 'response' | 'admin' | 'system'
        ahUser: '',              // distinct actor displayName filter ('' = all)
        ahSearch: '',            // doc/DRR substring filter
        ahDrawerDoc: null        // docNumber whose journey drawer is open
    };
    ...
    window.imsReviewLoad = function () {
        ...
        fetch('/api/ims-review/role?asDO=' + asDO, { credentials: 'same-origin' })
            ...
            .then(function (role) {
                ...
                _state.meta = role;
                if (role.view === 'admin') {
                    // Render the fast cached snapshot, then auto-pull live in the
                    // background so the tiles self-correct without a manual
                    // Refresh (docs cache is up to 60 min stale + owner-status
                    // enriches async). See imsReviewRefresh opts.
                    imsReviewRefresh(false, { thenRevalidate: true });
                } else {
                    loadMyQueue();
                }
            })
    ...
    // Freshness chip: 'updating' = cached shown while a live pull runs;
    // 'live' = live numbers in hand; 'cached' = live pull failed, cache shown.
    function setImsFreshness(kind, text) {
        var el = document.getElementById('imsFreshness');
        if (!el) return;
        var dotColor = kind === 'live' ? '#1F8A4C' : (kind === 'cached' ? '#B8342B' : '#C7801B');
        el.style.display = 'inline-flex';
        el.style.color = kind === 'live' ? '#1F8A4C' : '#6B7280';
        el.innerHTML = '<span style="display:inline-block; width:7px; height:7px; border-radius:50%; background:'
            + dotColor + ';"></span> ' + esc(text);
    }

    // opts (optional):
    //   background     — don't show the blocking spinner / toast; drive the
    //                    freshness chip instead (used for the auto-revalidate).
    //   thenRevalidate — after this (cached) render, auto-pull LIVE in the
    //                    background so the numbers self-correct without a
    //                    manual Refresh (the docs cache is up to 60 min stale).
    //   thenSettle     — after this LIVE pull, do one quiet follow-up read a few
    //                    seconds later to pick up the async owner-status (LDAP)
    //                    enrichment that drives the Need Owner tile.
    window.imsReviewRefresh = function (force, opts) {
        opts = opts || {};
        ...
        // sight while the user is looking at the toolbar. Use the top-right
        // pending toast so feedback is visible immediately. Background
        // (auto-revalidate) pulls skip the toast + spinner and use the chip.
        var pendingToast = (force && !background)
            ? (typeof showToast === 'function' ? showToast('Refreshing IMS Dashboard…', 'pending') : null)
            : null;
        if (background) {
            setImsFreshness('updating', 'updating to live…');
        } else {
        ...
        fetch(url, { credentials: 'same-origin' })
            ...
            .then(function (d) {
                ...
                // A live (force) pull, or the post-enrichment settle read, means
                // the numbers on screen are now live.
                if (force || opts.silent) {
                    setImsFreshness('live', 'live · as of ' + imsNowHHMM());
                } else if (opts.thenRevalidate) {
                    setImsFreshness('updating', 'cached · updating to live…');
                }
                // Cached first paint → auto-pull live in the background.
                if (opts.thenRevalidate) {
                    setTimeout(function () {
                        imsReviewRefresh(true, { background: true, thenSettle: true });
                    }, 150);
                }
                // Live pull kicked async LDAP enrichment on the server (writes
                // back to the docs cache); one quiet cached re-read a few seconds
                // later lets the Need Owner tile settle.
                if (opts.thenSettle) {
                    setTimeout(function () {
                        imsReviewRefresh(false, { background: true, silent: true });
                    }, 9000);
                }
            })
            .catch(function (e) {
                if (!background) {
                    ...
                    // Keep the cached numbers on screen; just note live failed.
                    setImsFreshness('cached', 'showing cached — live update failed');
                }
    ...
    window.imsReviewSwitchSubView = function (key) {
        ...
        var ah   = document.getElementById('imsReviewSubActionHistory');
        var bDocs = document.getElementById('imsSubTabDocs');
        var bOH   = document.getElementById('imsSubTabOwnerHistory');
        var bAH   = document.getElementById('imsSubTabActionHistory');
        var tools = document.getElementById('imsReviewAdminTools');
        var status = document.getElementById('imsReviewStatus');
        var view = (key === 'owner-history') ? 'oh' : (key === 'action-history') ? 'ah' : 'docs';
        if (docs)  docs.style.display = view === 'docs' ? '' : 'none';
        if (oh)    oh.style.display   = view === 'oh'   ? '' : 'none';
        if (ah)    ah.style.display   = view === 'ah'   ? '' : 'none';
        // The IMS-level toolbar (Window / Max overdue / Status / Refresh /
        // Export) is only relevant on the Documents view — hide it elsewhere.
        if (tools) tools.style.display = view === 'docs' ? '' : 'none';
        if (status) status.style.display = view === 'docs' ? '' : 'none';
        ...
        styleTab(bDocs, view === 'docs', '#4a6fa5');
        styleTab(bOH, view === 'oh', '#4a6fa5');
        styleTab(bAH, view === 'ah', '#4a6fa5');
        if (view === 'oh' && typeof window.ownerAuditInit === 'function') {
            window.ownerAuditInit();
        }
        if (view === 'ah') {
            imsActionHistoryLoad();
        }
    };
    ...
    function renderAdminTable(adminBox) {
        ...
        // PT-72: banner shown while the global locator is active so it's obvious
        // the table is searching across ALL tiles (not the selected one).
        var locatorActive = !!(_state.globalLocator || '').trim();
        var locatorBanner = locatorActive
            ? '<div style="background:#fff7e6; border:1px solid #f0d69a; border-radius:6px; padding:8px 14px; margin:0 0 12px; font-size:12.5px; color:#7a5b12; display:flex; align-items:center; justify-content:space-between; gap:12px;">'
              + '<span>&#128269; Searching <strong>all tiles</strong> for &ldquo;' + esc(_state.globalLocator.trim()) + '&rdquo; — ' + rows.length + ' match' + (rows.length === 1 ? '' : 'es') + '. Tile filters are ignored while searching.</span>'
              + '<a href="#" onclick="imsClearGlobalLocator(); return false;" style="color:#7a5b12; font-weight:600; text-decoration:none; white-space:nowrap;">&times; Clear search</a>'
              + '</div>'
            : '';
        // Hint: if this user is ALSO a DO/DM with pending items, offer the switch
        // PT-102 (Vikas 2026-07-23): the "You have N documents waiting for your
        // review as a DO/DM — Open your queue" banner was removed at his request.
        // Left as an empty string so the two render paths below still compose.
        var personalHint = '';
        if (rows.length === 0) {
            ...
            adminBox.innerHTML = locatorBanner + personalHint
                + '<p style="color:#6B7280; text-align:center; padding:20px 30px 6px;">' + (locatorActive ? 'No document matches that Doc / DRR / DCO number.' : emptyMsg) + '</p>'
        ...
        var html = locatorBanner + personalHint;
    ...
    window.imsBulkSend = function () {
        ...
        Promise.all(promises).then(function () {
            ...
            // One summarizing bucket toast — every bulk-sent doc lands in the
            // same tile (post-send = SENT_TO_DO). Classify a representative doc.
            if (sent > 0) {
                setTimeout(function () {
                    showBucketToast(sent + ' document' + (sent === 1 ? '' : 's') + ' are now in',
                        imsBucketKeyFor(docs[0], 'SENT_TO_DO'));
                }, 650);
            }
    ...
    // PT-72 global locator — search by Doc#, DRR#, or DCO# across ALL tiles.
    // Debounced so typing stays smooth; empty input restores the normal
    // tile-scoped view. When active, a small notice + Clear link is shown.
    var _locatorDebounce = null;
    window.imsGlobalLocator = function (val) {
        _state.globalLocator = val || '';
        if (_locatorDebounce) clearTimeout(_locatorDebounce);
        _locatorDebounce = setTimeout(function () {
            imsReviewRender();
            // Keep focus + caret in the search box across the re-render.
            var el = document.getElementById('imsGlobalLocatorInput');
            if (el) { el.focus(); el.value = _state.globalLocator; }
        }, 180);
    };
    window.imsClearGlobalLocator = function () {
        _state.globalLocator = '';
        var el = document.getElementById('imsGlobalLocatorInput');
        if (el) el.value = '';
        imsReviewRender();
    };
    ...
    function filterAdminRows() {
        ...
        // PT-72 global locator: when the search box has text, search EVERY row
        // regardless of tile (matching Doc#, DRR#, or DCO#) so a doc can be
        // found even when it's sitting in a tile the user isn't looking at.
        // This is exactly why "17-01-WW-01-00001 can't be found anywhere" —
        // the per-tile view hid it. An active locator bypasses the tile filter.
        var locator = (_state.globalLocator || '').trim().toLowerCase();
        if (locator) {
            return _state.adminRows.filter(function (r) {
                var hay = ((r.docNumber || '') + ' ' + (r.drrNumber || '') + ' '
                         + (r.agileDco || '') + ' ' + (r.description || '')).toLowerCase();
                return hay.indexOf(locator) >= 0;
            });
        }
        return _state.adminRows.filter(function (r) {
    ...
    window.imsSend = function (docNumber, drrNumber) {
        ...
        fetch('/api/ims-review/send', { method: 'POST', credentials: 'same-origin', body: fd })
            ...
            .then(function (q) {
                ...
                // Second toast: which bucket the doc now sits in (post-action state).
                var newStatus = (q && q.status) ? q.status : 'SENT_TO_DO';
                setTimeout(function () {
                    showBucketToast(docNumber + ' is now in', imsBucketKeyFor(docNumber, newStatus));
                }, 650);
                imsReviewRefresh();
            })
    ...
    window.imsCancel = function (docNumber, drrNumber) {
        ...
        fetch('/api/ims-review/cancel', { method: 'POST', credentials: 'same-origin', body: fd })
            .then(function (r) { return r.json(); })
            .then(function (q) {
                showToast('Review for ' + docNumber + ' cancelled.', 'ok');
                var newStatus = (q && q.status) ? q.status : 'CANCELLED';
                setTimeout(function () {
                    showBucketToast(docNumber + ' is now in', imsBucketKeyFor(docNumber, newStatus));
                }, 650);
                imsReviewRefresh();
          ...
          + 'the prior history stays in queue.jsonl as an audit trail.',
            ...
            fetch('/api/ims-review/reset', { method: 'POST', credentials: 'same-origin', body: fd })
                ...
                .then(function () {
                    ...
                    setTimeout(function () {
                        showBucketToast(docNumber + ' is now in', imsBucketKeyFor(docNumber, 'NOT_SENT'));
                    }, 650);
                    imsReviewRefresh();
                })
    ...
    // ======================================================================
    //  Action History sub-tab
    //  Derived, read-only feed of every user/system action on IMS docs.
    //  Backed by GET /api/ims-review/action-log?windowDays=30. Palettes and
    //  copy mirror the approved prototype (IMS Action History.dc.html).
    // ======================================================================

    // Action pill palette (bg / fg / category). ACT from the prototype.
    var IMS_ACT = {
        SEND:         { label: 'Send',            bg: '#e8f0fe',   fg: '#1a3a5c', cat: 'send' },
        RESEND:       { label: '↻ Resend',   bg: '#e8f0fe',   fg: '#1a3a5c', cat: 'send' },
        BULK_SEND:    { label: 'Bulk send',       bg: '#e8f0fe',   fg: '#1a3a5c', cat: 'send' },
        RESET:        { label: '⟲ Reset',    bg: '#fdeaea',   fg: '#B8342B', cat: 'admin' },
        UNLOCK:       { label: 'Unlock',          bg: '#fff3cd',   fg: '#856404', cat: 'admin' },
        CANCEL:       { label: 'Cancel',          bg: '#eef0f3',   fg: '#555555', cat: 'admin' },
        OWNER_EDIT:   { label: 'Owner edit',      bg: '#eef0f3',   fg: '#444444', cat: 'admin' },
        NO_CHANGE:    { label: 'DO: No change',   bg: '#e8f5e9',   fg: '#1F8A4C', cat: 'response' },
        NEEDS_CHANGE: { label: 'DO: Needs change', bg: '#7C3AED22', fg: '#5B21B6', cat: 'response' },
        OBS:          { label: 'DO: OBS',         bg: '#eef0f3',   fg: '#444444', cat: 'response' },
        NEED_HELP:    { label: 'DO: Need help',   bg: '#fdeaea',   fg: '#B8342B', cat: 'response' },
        DM_APPROVE:   { label: 'DM approve',      bg: '#e8f5e9',   fg: '#1F8A4C', cat: 'response' },
        SEND_BACK:    { label: '↩ Send back', bg: '#fdeaea',  fg: '#B8342B', cat: 'response' },
        DRR_CREATED:  { label: 'DRR created',     bg: '#FAFAF8',   fg: '#6B7280', cat: 'system' }
    };

    // Status pill palette — mirrors statusPill()'s switch verbatim, exposed as a
    // map so the compact "Moved" pills + journey drawer share the exact colors.
    var IMS_STATUS_PILL = {
        NOT_SENT:        { label: 'Not Sent',             bg: '#eef0f3',   fg: '#444' },
        SENT_TO_DO:      { label: 'Sent to DO',           bg: '#e8f0fe',   fg: '#1a3a5c' },
        SENT_TO_DM:      { label: 'Sent to DM',           bg: '#fff3cd',   fg: '#856404' },
        DM_APPROVED:     { label: 'DM Approved · Closed', bg: '#e8f5e9', fg: '#1F8A4C' },
        DO_NEEDS_CHANGE: { label: 'DO: Needs Change',     bg: '#7C3AED22', fg: '#5B21B6' },
        DO_NEED_HELP:    { label: 'Need Help',            bg: '#fdeaea',   fg: '#B8342B' },
        CANCELLED:       { label: 'Cancelled',            bg: '#eef0f3',   fg: '#888' }
    };

    function ahActPill(actionType) {
        var a = IMS_ACT[actionType] || { label: actionType, bg: '#eef0f3', fg: '#444' };
        return '<span style="display:inline-block; padding:2px 9px; border-radius:999px; font-size:11px; font-weight:600; background:'
             + a.bg + '; color:' + a.fg + ';">' + esc(a.label) + '</span>';
    }
    function ahStatusPillMini(status, bold) {
        var p = IMS_STATUS_PILL[status] || { label: status || '', bg: '#eef0f3', fg: '#444' };
        return '<span style="display:inline-block; padding:1px 7px; border-radius:999px; font-size:10.5px; background:'
             + p.bg + '; color:' + p.fg + ';' + (bold ? ' font-weight:600;' : '') + '">' + esc(p.label) + '</span>';
    }

    function ahDayLabel(epochMs) {
        ...
        var md = _MONTHS[d.getMonth()] + ' ' + d.getDate();
        var wd = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d.getDay()];
        if (diff === 0) return 'Today, ' + md;
        if (diff === 1) return 'Yesterday, ' + md;
        if (diff > 1 && diff < 7) return wd + ', ' + md;
        return md + ' ' + d.getFullYear();
    }
    function ahTime(epochMs) {
        var d = new Date(epochMs), h = d.getHours(), m = d.getMinutes();
        return (h < 10 ? '0' : '') + h + ':' + (m < 10 ? '0' : '') + m;
    }
    function ahMonthDayTime(epochMs) {
        var d = new Date(epochMs);
        return _MONTHS[d.getMonth()] + ' ' + d.getDate() + ', ' + ahTime(epochMs);
    }
    function ahFmtDwell(ms) {
        if (ms == null || ms < 0) return '';
        var mins = Math.floor(ms / 60000);
        if (mins < 60) return mins + 'm';
        var hrs = Math.floor(mins / 60);
        var days = Math.floor(hrs / 24);
        if (days >= 1) { var rh = hrs - days * 24; return days + 'd' + (rh ? ' ' + rh + 'h' : ''); }
        var rm = mins - hrs * 60;
        return hrs + 'h' + (rm ? ' ' + rm + 'm' : '');
    }
    function ahJourneyTitle(actionType) {
        switch (actionType) {
            case 'SEND':         return 'Sent to document owner';
            case 'RESEND':       return 'Reminder re-sent';
            case 'NO_CHANGE':    return 'DO responded: No change';
            case 'NEEDS_CHANGE': return 'DO responded: Needs change';
            case 'NEED_HELP':    return 'DO responded: Need help';
            case 'DM_APPROVE':   return 'DM approved — closed';
            case 'SEND_BACK':    return 'DM sent back to DO';
            case 'CANCEL':       return 'Review cancelled';
            case 'RESET':        return 'Reset to Not Sent';
            default:             return actionType || '';
        }
    }

    // Lazy fetch on first activation; re-render from cache thereafter.
    window.imsActionHistoryLoad = function () {
        var body = document.getElementById('imsActionHistoryBody');
        if (!body) return;
        if (_state.ahLoading) return;
        _state.ahLoading = true;
        // Re-fetch on every tab open so a reset / send / cancel done since the
        // last view shows up (the log is a lightweight queue.jsonl read). In-tab
        // filter/search changes still re-render from the cached _state.actionLog
        // without re-fetching. If we already have a payload, keep it on screen
        // while the fresh one loads underneath (no blank flash).
        if (_state.actionLog) {
            renderActionHistory();
        } else {
            body.innerHTML = '<div style="padding:40px; text-align:center; color:#6B7280;">'
                           + '<div class="spinner" style="margin:0 auto 12px;"></div>Loading action history…</div>';
        }
        fetch('/api/ims-review/action-log?windowDays=30', { credentials: 'same-origin' })
            .then(function (r) {
                if (!r.ok) return r.json().then(function (j) { throw new Error(j.error || ('HTTP ' + r.status)); });
                return r.json();
            })
            .then(function (d) {
                _state.actionLog = d;
                _state.ahLoading = false;
                renderActionHistory();
            })
            .catch(function (e) {
                _state.ahLoading = false;
                body.innerHTML = '<p style="color:#B8342B; padding:24px; text-align:center;">Could not load action history: '
                               + esc(e.message) + '</p>';
            });
    };

    window.imsActionHistoryRefresh = function () {
        _state.actionLog = null;
        imsActionHistoryLoad();
    };

    function ahFilteredEvents(data) {
        var q = (_state.ahSearch || '').trim().toLowerCase();
        return (data.events || []).filter(function (e) {
            var a = IMS_ACT[e.actionType];
            var cat = a ? a.cat : 'system';
            if (_state.ahCat !== 'all' && cat !== _state.ahCat) return false;
            if (_state.ahUser && e.displayName !== _state.ahUser) return false;
            if (q) {
                var doc = (e.docNumber || '').toLowerCase();
                var drr = (e.drrNumber || '').toLowerCase();
                if (doc.indexOf(q) < 0 && drr.indexOf(q) < 0) return false;
            }
            return true;
        });
    }
    ...
    function renderActionHistory() {
        var body = document.getElementById('imsActionHistoryBody');
        if (!body) return;
        var data = _state.actionLog || { events: [], stateCounts: {}, transitionCounts: {}, journeys: {} };
        var rows = ahFilteredEvents(data);
        // PT-102 (Vikas 2026-07-23): the "Where documents go" summary tiles at
        // the top of Action History were removed at his request. The table +
        // toolbar below are unchanged. (ahFlowMapHtml retained but now unused.)
        var html = '<div style="background:#fff; border:1px solid #E8E6DF; border-radius:8px; overflow:hidden;">'
                 +   ahToolbarHtml(data)
                 +   ahTableHtml(rows)
                 +   ahFooterHtml(rows.length)
                 + '</div>';
        if (_state.ahDrawerDoc) html += ahDrawerHtml();
        // Capture focus intent BEFORE innerHTML wipes the DOM (blur would fire
        // mid-replace, so a flag set by onblur can't be trusted here).
        var ae = document.activeElement;
        var keepFocus = !!(ae && ae.id === 'imsAhSearch');
        body.innerHTML = html;
        if (keepFocus) {
            var s = document.getElementById('imsAhSearch');
            if (s) { s.focus(); try { var v = s.value.length; s.setSelectionRange(v, v); } catch (e) {} }
        }
    }

    function ahFlowMapHtml(data) {
        var sc = data.stateCounts || {};
        var tc = data.transitionCounts || {};
        var stages = [
            { key: 'NOT_SENT',    label: 'Not Sent',              sub: 'queued for DCC send',      bg: '#eef0f3', fg: '#444',     border: '#E8E6DF', arrowLabel: 'send',    trans: 'send' },
            { key: 'SENT_TO_DO',  label: 'Sent to DO',            sub: 'awaiting owner response',  bg: '#e8f0fe', fg: '#1a3a5c', border: '#c9d9f0', arrowLabel: 'respond', trans: 'respond' },
            { key: 'SENT_TO_DM',  label: 'Sent to DM',            sub: 'awaiting DM approval',     bg: '#fff3cd', fg: '#856404', border: '#efe0a8', arrowLabel: 'approve', trans: 'approve' },
            { key: 'DM_APPROVED', label: 'DM Approved · Closed', sub: 'DCO to CCB / Release', bg: '#e8f5e9', fg: '#1F8A4C', border: '#c4e3cb', arrowLabel: null }
        ];
        var boxes = '';
        stages.forEach(function (st) {
            boxes += '<div style="display:flex; align-items:stretch; flex:1;">'
                   +   '<div style="flex:1; border:1px solid ' + st.border + '; border-radius:6px; background:' + st.bg + '; padding:10px 12px; display:flex; flex-direction:column; gap:2px;">'
                   +     '<div style="font-size:12px; font-weight:600; color:' + st.fg + ';">' + esc(st.label) + '</div>'
                   +     '<div style="font-family:\'IBM Plex Serif\',Georgia,serif; font-size:24px; font-weight:600; color:#1a1d23;">' + (sc[st.key] || 0) + '</div>'
                   +     '<div style="font-size:10.5px; color:#6B7280;">' + esc(st.sub) + '</div>'
                   +   '</div>';
            if (st.arrowLabel) {
                boxes += '<div style="width:74px; flex-shrink:0; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:1px;">'
                       +   '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10px; color:#6B7280;">' + esc(st.arrowLabel) + '</div>'
                       +   '<div style="font-size:15px; color:#9AA0A6; line-height:1;">→</div>'
                       +   '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10px; color:#1a1d23; font-weight:600;">' + (tc[st.trans] || 0) + '</div>'
                       + '</div>';
            }
            boxes += '</div>';
        });
        var branches = [
            { label: 'Needs Change',        sub: 'DO redlines in flight',   dot: '#5B21B6', count: sc.DO_NEEDS_CHANGE || 0 },
            { label: 'Need Help',           sub: 'DCC handling',            dot: '#B8342B', count: sc.DO_NEED_HELP || 0 },
            { label: 'Reset ↩ Not Sent', sub: 'this window',           dot: '#B8342B', count: tc.reset || 0 },
            { label: 'Cancelled',           sub: 'superseded / withdrawn',  dot: '#888',    count: sc.CANCELLED || 0 }
        ];
        var chips = '';
        branches.forEach(function (b) {
            chips += '<div style="display:flex; align-items:center; gap:7px; border:1px dashed #E8E6DF; border-radius:999px; padding:4px 12px 4px 8px; background:#fff;">'
                   +   '<span style="width:7px; height:7px; border-radius:50%; background:' + b.dot + '; display:inline-block;"></span>'
                   +   '<span style="font-size:11.5px; color:#444;">' + esc(b.label) + '</span>'
                   +   '<span style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:11px; font-weight:600;">' + b.count + '</span>'
                   +   '<span style="font-size:10.5px; color:#9AA0A6;">' + esc(b.sub) + '</span>'
                   + '</div>';
        });
        return '<div style="background:#fff; border:1px solid #E8E6DF; border-radius:8px; margin-bottom:14px; overflow:hidden;">'
             +   '<div style="padding:8px 14px; background:#FAFAF8; border-bottom:1px solid #E8E6DF; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:6px;">'
             +     '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10.5px; letter-spacing:.08em; text-transform:uppercase; color:#6B7280;">Where documents go · last 30 days</div>'
             +     '<div style="font-size:11px; color:#9AA0A6;">counts = docs currently in each state · arrows = transitions this window</div>'
             +   '</div>'
             +   '<div style="padding:18px 14px 8px; overflow-x:auto;">'
             +     '<div style="display:flex; align-items:stretch; gap:0; min-width:860px;">' + boxes + '</div>'
             +     '<div style="display:flex; gap:10px; margin-top:12px; padding-bottom:10px; flex-wrap:wrap;">' + chips + '</div>'
             +   '</div>'
             + '</div>';
    }

    function ahToolbarHtml(data) {
        var cats = [
            { key: 'all', label: 'All actions' }, { key: 'send', label: 'Sends' },
            { key: 'response', label: 'Responses' }, { key: 'admin', label: 'Resets & admin' },
            { key: 'system', label: 'System' }
        ];
        var chips = cats.map(function (c) {
            var on = _state.ahCat === c.key;
            return '<button onclick="imsAhSetCat(\'' + c.key + '\')" style="padding:4px 11px; font-size:11.5px; border-radius:999px; cursor:pointer; font-family:inherit; border:1px solid '
                 + (on ? '#2c3e50' : '#E8E6DF') + '; background:' + (on ? '#2c3e50' : '#fff') + '; color:' + (on ? '#fff' : '#444')
                 + '; font-weight:' + (on ? '600' : '400') + ';">' + esc(c.label) + '</button>';
        }).join('');
        var users = [];
        (data.events || []).forEach(function (e) { if (e.displayName && users.indexOf(e.displayName) < 0) users.push(e.displayName); });
        users.sort();
        var opts = '<option value="">All users</option>' + users.map(function (u) {
            return '<option value="' + esc(u) + '"' + (_state.ahUser === u ? ' selected' : '') + '>' + esc(u) + '</option>';
        }).join('');
        return '<div style="padding:10px 14px; border-bottom:1px solid #E8E6DF; display:flex; align-items:center; gap:10px; flex-wrap:wrap;">'
             +   '<div style="display:flex; gap:4px; flex-wrap:wrap;">' + chips + '</div>'
             +   '<div style="flex:1;"></div>'
             +   '<select onchange="imsAhSetUser(this.value)" style="padding:5px 8px; font-size:12px; border:1px solid #E8E6DF; border-radius:4px; background:#fff; color:#444; font-family:inherit;">' + opts + '</select>'
             +   '<input id="imsAhSearch" value="' + esc(_state.ahSearch || '') + '" oninput="imsAhSetSearch(this.value)" placeholder="Filter by doc / DRR…" style="padding:5px 9px; font-size:12px; border:1px solid #E8E6DF; border-radius:4px; width:170px; font-family:inherit; color:#1a1d23;">'
             + '</div>';
    }

    function ahTh(label) {
        return '<th style="text-align:left; padding:8px 10px; font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10px; letter-spacing:.08em; text-transform:uppercase; color:#6B7280; font-weight:500; border-bottom:1px solid #E8E6DF; white-space:nowrap;">' + label + '</th>';
    }
    function ahTableHtml(rows) {
        if (!rows.length) {
            return '<div style="padding:34px; text-align:center; color:#9AA0A6; font-size:12.5px;">No actions match the current filters.</div>';
        }
        var head = '<thead><tr style="background:#FAFAF8;">'
                 + ahTh('When') + ahTh('User') + ahTh('Action') + ahTh('Document') + ahTh('Moved') + ahTh('Details')
                 + '</tr></thead>';
        var body = rows.map(function (e) {
            var docSafe = esc(e.docNumber || '').replace(/'/g, "\\'");
            var hasJourney = _state.actionLog && _state.actionLog.journeys && _state.actionLog.journeys[e.docNumber];
            var docCell = hasJourney
                ? '<a href="#" onclick="imsAhOpenJourney(\'' + docSafe + '\'); return false;" style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:11.5px; font-weight:600; color:#4a6fa5; text-decoration:none;">' + esc(e.docNumber || '') + '</a>'
                : '<span style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:11.5px; font-weight:600;">' + esc(e.docNumber || '') + '</span>';
            var moved = (e.fromStatus && e.toStatus)
                ? '<span style="display:inline-flex; align-items:center; gap:5px;">' + ahStatusPillMini(e.fromStatus, false)
                    + '<span style="color:#9AA0A6; font-size:12px;">→</span>' + ahStatusPillMini(e.toStatus, true) + '</span>'
                : '<span style="color:#C9C7C0;">—</span>';
            return '<tr style="border-bottom:1px solid #F1F0EB;">'
                 +   '<td style="padding:8px 14px; white-space:nowrap; vertical-align:top;">'
                 +     '<div style="font-weight:600;">' + esc(ahDayLabel(e.epochMs)) + '</div>'
                 +     '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:11px; color:#6B7280;">' + esc(ahTime(e.epochMs)) + '</div>'
                 +   '</td>'
                 +   '<td style="padding:8px 10px; vertical-align:top; white-space:nowrap;">'
                 +     '<div>' + esc(e.displayName || '') + '</div>'
                 +     '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10.5px; color:#9AA0A6;">' + esc(e.actorRole || '') + '</div>'
                 +   '</td>'
                 +   '<td style="padding:8px 10px; vertical-align:top; white-space:nowrap;">' + ahActPill(e.actionType) + '</td>'
                 +   '<td style="padding:8px 10px; vertical-align:top; white-space:nowrap;">' + docCell
                 +     '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10.5px; color:#9AA0A6;">' + esc(e.drrNumber || '') + '</div>'
                 +   '</td>'
                 +   '<td style="padding:8px 10px; vertical-align:top; white-space:nowrap;">' + moved + '</td>'
                 +   '<td style="padding:8px 14px; vertical-align:top; color:#444; max-width:340px;">' + esc(e.details || '') + '</td>'
                 + '</tr>';
        }).join('');
        return '<div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:12px;">' + head + '<tbody>' + body + '</tbody></table></div>';
    }

    function ahFooterHtml(count) {
        var days = (_state.actionLog && _state.actionLog.windowDays) || 30;
        return '<div style="padding:8px 14px; border-top:1px solid #E8E6DF; background:#FAFAF8; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">'
             +   '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:11px; color:#6B7280;">' + count + ' action' + (count === 1 ? '' : 's') + ' · last ' + days + ' days · newest first</div>'
             +   '<div style="font-size:11px; color:#9AA0A6;">Click a document number to trace its journey</div>'
             + '</div>';
    }

    function ahDrawerHtml() {
        var doc = _state.ahDrawerDoc;
        var log = _state.actionLog || {};
        var j = (log.journeys && log.journeys[doc]) || [];
        var drr = '';
        (log.events || []).some(function (e) { if (e.docNumber === doc && e.drrNumber) { drr = e.drrNumber; return true; } return false; });
        var last = j.length ? j[j.length - 1] : null;
        var nowStatus = last ? last.toStatus : 'NOT_SENT';
        var nodes = j.map(function (n, i, arr) {
            var p = IMS_STATUS_PILL[n.toStatus] || { fg: '#444' };
            var dwell = (i < arr.length - 1) ? ahFmtDwell(arr[i + 1].epochMs - n.epochMs) : '';
            return '<div style="display:flex; gap:12px;">'
                 +   '<div style="display:flex; flex-direction:column; align-items:center; width:14px; flex-shrink:0;">'
                 +     '<div style="width:9px; height:9px; border-radius:50%; background:' + p.fg + '; margin-top:4px; flex-shrink:0;"></div>'
                 +     (i < arr.length - 1 ? '<div style="width:1px; flex:1; background:#E8E6DF;"></div>' : '')
                 +   '</div>'
                 +   '<div style="padding-bottom:18px; flex:1;">'
                 +     '<div style="display:flex; align-items:baseline; gap:8px; flex-wrap:wrap;">'
                 +       '<span style="font-size:12.5px; font-weight:600;">' + esc(ahJourneyTitle(n.actionType)) + '</span>'
                 +       ahStatusPillMini(n.toStatus, false)
                 +     '</div>'
                 +     '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10.5px; color:#9AA0A6; margin-top:2px;">' + esc(ahMonthDayTime(n.epochMs)) + ' · ' + esc((n.displayName || '') + ' · ' + (n.actorRole || '')) + '</div>'
                 +     (n.note ? '<div style="font-size:11.5px; color:#444; margin-top:4px;">' + esc(n.note) + '</div>' : '')
                 +     (dwell ? '<div style="display:inline-block; margin-top:6px; font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10px; color:#6B7280; background:#FAFAF8; border:1px solid #E8E6DF; border-radius:4px; padding:2px 7px;">' + esc(dwell) + ' in this state</div>' : '')
                 +   '</div>'
                 + '</div>';
        }).join('');
        return '<div onclick="imsAhCloseDrawer()" style="position:fixed; inset:0; background:rgba(15,23,32,.45); z-index:9100;"></div>'
             + '<div style="position:fixed; top:0; right:0; bottom:0; width:420px; max-width:92vw; background:#fff; z-index:9101; box-shadow:-8px 0 30px rgba(15,23,32,.18); display:flex; flex-direction:column;">'
             +   '<div style="padding:16px 18px 12px; border-bottom:1px solid #E8E6DF; display:flex; align-items:flex-start; justify-content:space-between; gap:10px;">'
             +     '<div>'
             +       '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10.5px; letter-spacing:.08em; text-transform:uppercase; color:#6B7280; margin-bottom:3px;">Document journey</div>'
             +       '<div style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:15px; font-weight:600;">' + esc(doc) + '</div>'
             +       '<div style="font-size:11.5px; color:#6B7280; margin-top:2px;">' + esc(drr) + ' · currently ' + ahStatusPillMini(nowStatus, true) + '</div>'
             +     '</div>'
             +     '<button onclick="imsAhCloseDrawer()" style="background:none; border:none; font-size:18px; color:#6B7280; cursor:pointer; line-height:1; padding:2px;">×</button>'
             +   '</div>'
             +   '<div style="flex:1; overflow-y:auto; padding:18px 18px 24px;">' + (nodes || '<div style="color:#9AA0A6; font-size:12px;">No journey events.</div>') + '</div>'
             + '</div>';
    }

    window.imsAhSetCat = function (key) { _state.ahCat = key; renderActionHistory(); };
    window.imsAhSetUser = function (val) { _state.ahUser = val || ''; renderActionHistory(); };
    window.imsAhSetSearch = function (val) { _state.ahSearch = val || ''; renderActionHistory(); };
    window.imsAhOpenJourney = function (doc) { _state.ahDrawerDoc = doc; renderActionHistory(); };
    window.imsAhCloseDrawer = function () { _state.ahDrawerDoc = null; renderActionHistory(); };

    // ======================================================================
    //  Feature 2 — "which bucket" toast (Documents tab)
    //  Fires ~650ms AFTER the existing confirmation toast: a white card
    //  naming the tile the doc has moved into. Destination is computed via
    //  the shared classifier on the post-action row (no hard-coded mapping).
    // ======================================================================

    /** Look up display metadata for a 'group.tile' key from TILE_GROUPS. */
    function ahTileMetaFor(key) {
        for (var gi = 0; gi < TILE_GROUPS.length; gi++) {
            var g = TILE_GROUPS[gi];
            for (var ti = 0; ti < g.tiles.length; ti++) {
                var t = g.tiles[ti];
                if (g.group + '.' + t.tile === key) {
                    return { color: t.color, tileLabel: t.label, groupLabel: g.label, sub: t.sub };
                }
            }
        }
        return null;
    ...
    /** Classify a doc's destination tile from its POST-action status, reusing
     *  the cached full row so the classifier sees DRR/DCO/owner fields. */
    function imsBucketKeyFor(docNumber, newStatus) {
        var row = null;
        (_state.adminRows || []).forEach(function (r) { if (r.docNumber === docNumber) row = r; });
        var base = row ? Object.assign({}, row) : { docNumber: docNumber, drrNumber: '', owners: [] };
        base.status = newStatus;
        try { return ImsClassify.imsTileKey(base, _state.drrCreatedAfter); } catch (e) { return null; }
    }

    /** White two-line "now in <bucket>" card. Stacks below the slate toast in
     *  the same top-right host; auto-dismisses in ~6.5s. */
    function showBucketToast(titleText, tileKey) {
        var meta = tileKey ? ahTileMetaFor(tileKey) : null;
        if (!meta) return;
        if (!document.getElementById('imsBucketToastStyles')) {
            var st = document.createElement('style');
            st.id = 'imsBucketToastStyles';
            st.textContent = '@keyframes imsBucketIn{from{opacity:0;transform:translateY(8px);}to{opacity:1;transform:translateY(0);}}'
                           + '@keyframes imsBucketOut{from{opacity:1;transform:translateY(0);}to{opacity:0;transform:translateY(8px);}}';
            document.head.appendChild(st);
        }
        // Reuse the showToast host so the bucket card stacks below the slate one.
        var host = document.getElementById('imsToastHost');
        if (!host) {
            host = document.createElement('div');
            host.id = 'imsToastHost';
        ...
        var el = document.createElement('div');
        el.style.cssText = 'display:flex; align-items:flex-start; gap:9px; background:#fff; color:#1a1d23; border:1px solid #E8E6DF;'
                         + ' border-radius:6px; padding:10px 14px; box-shadow:0 6px 20px rgba(15,23,32,.18); max-width:360px;'
                         + ' pointer-events:auto; animation:imsBucketIn 180ms cubic-bezier(.2,.7,.3,1);';
        el.innerHTML = '<span style="font-size:13px; line-height:1.3;">⇥</span>'
                     + '<div>'
                     +   '<div class="imsbt-title" style="font-size:12.5px; font-weight:600; line-height:1.35;"></div>'
                     +   '<div style="font-size:11.5px; opacity:.85; margin-top:2px; display:flex; align-items:center; gap:6px;">'
                     +     '<span style="width:7px; height:7px; border-radius:50%; background:' + meta.color + '; display:inline-block; flex-shrink:0;"></span>'
                     +     '<span class="imsbt-sub"></span>'
                     +   '</div>'
                     + '</div>';
        // Fill via classed nodes (avoid HTML injection from doc numbers, and
        // don't clobber the inner DOM the way a generic 'div > div' select did).
        el.querySelector('.imsbt-title').textContent = titleText;
        el.querySelector('.imsbt-sub').textContent = meta.groupLabel + ' › ' + meta.tileLabel + ' — ' + meta.sub;
        host.appendChild(el);
        setTimeout(function () {
            if (!el.parentNode) return;
            el.style.animation = 'imsBucketOut 160ms ease-in forwards';
            setTimeout(function () { if (el.parentNode) el.parentNode.removeChild(el); }, 180);
        }, 6500);
    }
    // Exposed for the mutation handlers below.
    window.imsBucketKeyFor = imsBucketKeyFor;
    window.showBucketToast = showBucketToast;
})();
