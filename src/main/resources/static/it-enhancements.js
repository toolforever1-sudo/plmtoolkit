...
(function () {
    ...
    function renderRestartEcnDialog(data) {
        ...
        pop.innerHTML =
            ...
            '</div>' +
            '<div style="font-size:11.5px; color:#6B7280; margin:4px 0 6px; line-height:1.4;">Each file references the enhancement ECN(s) it belongs to &mdash; written into the file description as ECN #, ECN # on the Attachments tab in Agile. .jar and .properties files also need an IT attachment type.</div>' +
        ...
        function descPreview(f) {
            ...
            var base = refs.join(', ');   // just the ECN number(s)
            if (f.note && f.note.trim()) return (base ? base + ' — ' : '') + f.note.trim();
            return base;
        }
        function addFiles(list) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                reFiles.push({ id: ++fileSeq, file: file, ecns: {}, type: '', note: '' });
            }
        ...
        function renderFiles() {
            ...
            filesEl.innerHTML = reFiles.map(function (f) {
                ...
                var staleRefs = Object.keys(f.ecns).filter(function (e) { return f.ecns[e] && !selected[e]; });
                var refN = Object.keys(f.ecns).filter(function (e) { return f.ecns[e] && selected[e]; }).length;
                var valid = refN > 0 && (opts.length === 0 || f.type);
                ...
                return '<div style="border:1px solid ' + (valid ? '#E8E6DF' : '#E4A0A0') + '; border-radius:8px; padding:10px 12px; margin-bottom:8px; background:#fff;">' +
                    '<div style="display:flex; justify-content:space-between; align-items:center;">' +
                        '<span style="min-width:0;"><span style="color:#9CA3AF; margin-right:6px;">&#128206;</span><b style="font-size:13px; word-break:break-all;">' + escapeHtml(f.file.name) + '</b> <span style="color:#9CA3AF; font-size:11px;">' + humanSize(f.file.size) + '</span></span>' +
                        '<span data-fid="' + f.id + '" class="re-frm" title="remove" style="cursor:pointer; color:#9CA3AF; font-size:16px; font-weight:700; padding-left:8px;">&times;</span>' +
                    '</div>' +
                    '<div style="margin-top:8px;"><span style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#9CA3AF; margin-right:6px;">for ecn #</span>' + (chips || '<span style="color:#C7801B; font-size:11px;">check ECNs above to tag this file</span>') + '</div>' +
                    (staleRefs.length ? '<div style="color:#B8342B; font-size:11px; margin-top:4px;">re-tag this file — references unbundled ECN' + (staleRefs.length > 1 ? 's' : '') + ': ' + escapeHtml(staleRefs.join(', ')) + '</div>' : '') +
            ...
            Array.prototype.forEach.call(filesEl.querySelectorAll('.re-frm'), function (x) {
                x.addEventListener('click', function () { reFiles = reFiles.filter(function (y) { return y.id != x.getAttribute('data-fid'); }); renderFiles(); });
            });
            Array.prototype.forEach.call(filesEl.querySelectorAll('.re-ftype'), function (s) {
                s.addEventListener('change', function () { var f = reFiles.filter(function (x) { return x.id == s.getAttribute('data-fid'); })[0]; if (f) { f.type = s.value; refreshCreate(); } });
            });
    ...
    function openAddFilesForm(rec) {
        ...
        fetch('/api/it-enhancements/restart-ecn-candidates', { credentials: 'same-origin' })
            ...
            .then(function (d) {
                ...
                function descPrev(f) {
                    var base = Object.keys(f.ecns).filter(function (e) { return f.ecns[e]; }).join(', ');
                    if (f.note && f.note.trim()) return (base ? base + ' — ' : '') + f.note.trim();
                    return base;
                }
                function render() {
                    var rows = files.map(function (f) {
                        ...
                        return '<div style="border:1px solid ' + (valid(f) ? '#E8E6DF' : '#E4A0A0') + '; border-radius:8px; padding:10px 12px; margin-bottom:8px;">' +
                            '<div style="display:flex; justify-content:space-between; align-items:center;"><span style="min-width:0;"><span style="color:#9CA3AF; margin-right:6px;">&#128206;</span><b style="font-size:12.5px; word-break:break-all;">' + escapeHtml(f.file.name) + '</b></span><span data-fid="' + f.id + '" class="reAfRm" style="cursor:pointer; color:#9CA3AF; font-size:16px; font-weight:700; padding-left:8px;">&times;</span></div>' +
                            '<div style="margin-top:8px;"><span style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#9CA3AF; margin-right:6px;">for ecn #</span>' + (chips || '<span style="color:#C7801B; font-size:11px;">no bundled ECNs</span>') + '</div>' +
                            (typeSel ? '<div style="margin-top:8px;">' + typeSel + '</div>' : '') +
                            '<input data-fid="' + f.id + '" class="reAfNote" placeholder="Description note (optional)" value="' + escapeAttr(f.note || '') + '" style="display:block; width:100%; box-sizing:border-box; margin-top:8px; font-size:12px; padding:6px 8px; border:1px solid #E8E6DF; border-radius:6px;">' +
                            '<div class="reAfDesc" data-fid="' + f.id + '" style="font-family:\'IBM Plex Mono\',Consolas,monospace; font-size:11px; color:#6B7280; margin-top:8px; word-break:break-word;">description: ' + escapeHtml(descPrev(f)) + '</div>' +
                        '</div>';
                    }).join('');
                    ...
                    host.querySelector('#reAfInput').addEventListener('change', function () {
                        var skipped = [];
                        for (var i = 0; i < this.files.length; i++) { var fl = this.files[i]; if (fl.size > maxMb * 1048576) { skipped.push(fl.name + ' (' + (fl.size / 1048576).toFixed(1) + ' MB)'); continue; } files.push({ id: ++seq, file: fl, ecns: {}, type: '', note: '' }); }
                    ...
                    Array.prototype.forEach.call(host.querySelectorAll('.reAfChip'), function (ch) {
                        ch.addEventListener('click', function () { var f = files.filter(function (x) { return x.id == ch.getAttribute('data-fid'); })[0]; if (!f) return; var e = ch.getAttribute('data-ecn'); if (f.ecns[e]) delete f.ecns[e]; else f.ecns[e] = true; render(); });
                    });
                    ...
                    Array.prototype.forEach.call(host.querySelectorAll('.reAfNote'), function (n) {
                        n.addEventListener('input', function () {
                            var f = files.filter(function (x) { return x.id == n.getAttribute('data-fid'); })[0]; if (!f) return;
                            f.note = n.value;
                            var d = host.querySelector('.reAfDesc[data-fid="' + f.id + '"]');
                            if (d) d.textContent = 'description: ' + descPrev(f);
                        });
                    });
                    Array.prototype.forEach.call(host.querySelectorAll('.reAfRm'), function (x) {
                        x.addEventListener('click', function () { files = files.filter(function (y) { return y.id != x.getAttribute('data-fid'); }); render(); });
                    });
