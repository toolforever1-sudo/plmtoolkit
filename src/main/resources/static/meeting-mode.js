  ...
  function meetingMain(live) {
    ...
    var railRows = S.items.filter(function (it) { return itemMatchesFilter(it, S.railFilter); }).map(function (it) {
      ...
      return '<div data-search="' + esc(ds) + '" data-requestor="' + esc(lc(it.requestor)) + '" data-priority="' + esc(lc(it.priority)) + '" data-status="' + esc(lc(it.status)) + '" onclick="MeetingMode._select(\'' + esc(it.id) + '\')" style="padding:11px 13px;border-bottom:1px solid var(--line);cursor:pointer;border-left:3px solid ' + tone + ';background:' + (seld ? 'var(--accent-2)' : '#fff') + '">'
        ...
        + (bs.length ? '<div style="font-family:var(--font-mono);font-size:9.5px;color:var(--accent-ink);margin-top:4px">● ' + bs.length + (bs.length === 1 ? ' update' : ' updates') + '</div>' : '')
        + '</div>';
    }).join('');
    ...
    var detail = '<div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap">'
      + ecnA(sel.id, sel.id, 'font-family:var(--font-mono);font-size:14px;color:var(--accent);text-decoration:none;font-weight:500') + badges + prioBadge(sel.priority) + '</div>'
    ...
    var selB = bubblesOf(sel.id);
    if (selB.length) {
      detail += '<div style="margin-top:16px"><div style="font-family:var(--font-mono);font-size:9.5px;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-3)">Captured on this item</div>'
        + '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px">'
        + selB.map(function (b) {
            var ag = b.kind === 'AGILE';
            return '<span style="display:inline-flex;font-family:' + (ag ? 'var(--font-mono)' : 'var(--font-sans)') + ';font-size:10px;padding:3px 8px;border-radius:5px;'
              + (ag ? 'background:var(--accent-2);color:var(--accent-ink);border-left:2px solid var(--accent)' : 'background:var(--warn-bg);color:#6a4a12;border-left:2px dashed #b9892f') + '">'
              + (ag ? '→ Agile · ' : '✦ Note · ') + esc(b.text) + '</span>';
          }).join('') + '</div></div>';
    }
